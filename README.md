...
The original dataset contains complete student information, including skill scores, skill-gap information, overall score, and training recommendation.

The Feast feature dataset contains the features required by the feature store along with:
...
The features are stored in Parquet format for historical retrieval.

## Offline Store

The offline store is used to keep historical feature data. It supports historical feature retrieval for Machine Learning training.

## Online Store

The online store keeps the latest materialized features and allows fast feature retrieval during prediction.

The project uses SQLite as the online store.

## Purpose of `feast apply`
...
registers the Feast entities, FeatureViews, and Feature Services defined in the Feast configuration.

## Purpose of Materialization

Materialization copies feature data from the offline source into the online store so that features can be retrieved for online prediction.

## Advantages of Feast

Using Feast provides a common feature management layer for training and prediction.

Advantages include:

1. Consistent features between training and prediction.
2. Historical feature retrieval for model training.
3. Online feature retrieval for prediction.
4. Centralized feature definitions.
5. Reduced risk of training-serving feature differences.

## Limitations

1. The current dataset is synthetic and may not fully represent real student performance.
2. The current dataset contains a fixed set of skills and limited industry evidence.

## Future Improvements

1. Add more real-world student and industry skill data.
2. Update skills based on current job-market requirements.
3. Add features from certifications, projects, internships, and coding assessments.
4. Add regularly updated industry skill requirements.
5. Experiment with advanced Machine Learning or Deep Learning models.
6. Create additional Feast FeatureViews for different skill categories.
...
```text
231FA04140-MLOps-Feast-SkillGap/
|
|-- README.md
|-- MLOPS.ipynb
...
This project demonstrates the complete basic workflow of a Feast-based feature store for a curriculum-industry skill-gap problem.
